...
class RegisterForm(FlaskForm):
    username = StringField("Username", validators=[InputRequired(message="Username Field Required"),
                                                   Length(
                                                       min=3, max=30, message="Username Must Contain 3 to 30 Characters"),
                                                   Regexp(
                                                       "^[A-Za-z][A-Za-z0-9_]{2,29}$", message="Username must start with a letter and can only contain letter numbers and underscores"),
                                                   check_username_dup])

    password = PasswordField("Password", validators=[InputRequired(message="Password Field Required"),
                                                     EqualTo(
                                                         "cpassword", message="Passwords Don't Match"),
                                                     Regexp(
                                                         "^\S{8,40}$", message="Password Cannot Contain Spaces"),
                                                     Length(min=8, max=40, message="Password Must Contain 8 to 40 Characters")])

    cpassword = PasswordField("Confirm Password", validators=[
                              InputRequired(message="Confirm Password Field Required")])
...
class SigninForm(FlaskForm):
    username = StringField("Username", validators=[InputRequired(
        message="Username Field Required"), validate_usermame])
    password = PasswordField("Password", validators=[InputRequired(
        message="Password Field Required"), validate_password])
...
class SearchPoemsForm(FlaskForm):
    ...
    filters = SelectField("filter", choices=[("author", "Search By Author"), ("title", "Search By Title"), (
        "lines", "Search By Poem Verse"), ("linecount", "Search By Poem Length")])
    search = SubmitField()
    sort_by = SelectField("sort_by", choices=[("sort_by", "Sort By"), ("shortest", "Shortest"), ("longest", "Longest"), (
        "author", "Author (A-Z)"), ("author_reverse", "Author (Z-A)"), ("title", "Title (A-Z)"), ("title_reverse", "Title (Z-A)")])


class RhymesForm(FlaskForm):
    query = StringField("query")
    filters = SelectField("filter", choices=[("rel_rhy", "Search For Rhymes"), (
        "rel_syn", "Search For Synonyms"), ("rel_ant", "Search For Antonyms")])
    search = SubmitField()
